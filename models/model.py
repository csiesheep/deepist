...
class DeepIST(object):
    ...
    def __init__(self, image_input, length_input, max_len, trainable=True, dropout=0.5, size=200, channel=1, cnn2d_dim=1024, cnn1d_dim=1024, cnn2d_convs=[64, 128, 256, 512], filter_size=3):
        self.data_dict = None
...
class Simple1dCNN(object):
    ...
    def __init__(self, image_input,
                 trainable=True, dropout=0.5, input_dim=1024, output_dim=1024, filter_size=3):
        self.data_dict = None
    ...
    def build(self, train_mode=None):
        self.conv1_1 = self.conv_layer(self.image_input, self.input_dim, self.input_dim, self.filter_size, "conv1_1")
...
#       self.fc6 = self.fc_layer(self.pool1, int(self.pool1.shape[1]*self.pool1.shape[2]), self.output_dim, "fc6") #200, 1conv
        ...
        self.conv2_1 = self.conv_layer(self.pool1, self.input_dim, self.input_dim, self.filter_size, "conv2_1")
...
#       self.conv3_1 = self.conv_layer(self.pool2, self.input_dim, self.input_dim, filter_size, "conv3_1")
...
#       self.fc6 = self.fc_layer(self.pool3, int(self.pool3.shape[1]*self.pool3.shape[2]), self.output_dim, "fc6") #200, 3conv
    ...
    def conv_layer(self, bottom, in_channels, out_channels, filter_size, name):
        with tf.variable_scope(name):
...
class PathCNN(object):
    ...
    def __init__(self, image_input,
                 trainable=True, dropout=0.5, channel=1, embedding_dim=2048,
                 convs=[64, 128, 256, 512], filter_size=3):
        self.data_dict = None
    ...
    def build(self, train_mode=None):
        ...
        # conv->max + conv->avg
        self.conv1_avg = self.conv_layer(self.image_input, self.channel, self.convs[0], self.filter_size, "conv1_avg")
        self.conv1_max = self.conv_layer(self.image_input, self.channel, self.convs[0], self.filter_size, "conv1_max")
        ...
        self.conv2_avg = self.conv_layer(self.pool1, self.convs[0]*2, self.convs[1], self.filter_size, "conv2_avg")
        self.conv2_max = self.conv_layer(self.pool1, self.convs[0]*2, self.convs[1], self.filter_size, "conv2_avg")
        ...
        self.conv3_avg = self.conv_layer(self.pool2, self.convs[1]*2, self.convs[2], self.filter_size, "conv3_avg")
        self.conv3_max = self.conv_layer(self.pool2, self.convs[1]*2, self.convs[2], self.filter_size, "conv3_avg")
        ...
        self.conv4_avg = self.conv_layer(self.pool3, self.convs[2]*2, self.convs[3], self.filter_size, "conv4_avg")
        self.conv4_max = self.conv_layer(self.pool3, self.convs[2]*2, self.convs[3], self.filter_size, "conv4_avg")
...
##      self.pool4 = self.max_pool(self.conv4_1, 'pool4')
 ...
 #      self.fc6 = self.fc_layer(self.pool4, 7*7*self.convs[-1], self.embedding_dim, "fc6") #100
    ...
    def conv_layer(self, bottom, in_channels, out_channels, filter_size, name):
        with tf.variable_scope(name):
            filters, biases = self.get_conv_var(filter_size, in_channels, out_channels, name)
